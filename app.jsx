...
// Firebase safe configuration fallback
let firebaseConfig = {
  apiKey: "",
...
// Standard Time String Templates
...
// Detailed timezone parser extracting KST, EST, BRT, and ST
const parseDetailedEventTimes = (timeStr) => {
  ...
  if (stMatch) st = `ST ${stMatch[1]}`;

  const estMatch = timeStr.match(/EST\s*(\d{1,2}:\d{2}\*?)/i);
  if (estMatch) est = `EST ${estMatch[1]}`;

  const brtMatch = timeStr.match(/BRT\s*(\d{1,2}:\d{2}\*?)/i);
  if (brtMatch) brt = `BRT ${brtMatch[1]}`;

  return { kst, est, brt, st };
};

// Utility to adjust time strings by ±N minutes
...
// Initial Default Schedule Generator (Through Dec 31, 2026)
const buildInitialDefaultSchedule = () => {
  ...
  // 1. Military Exercise (MG): 3-day interval rotation
  ...
  // 2. Zombie Siege (ZS): 3-day interval rotation
  ...
  // 3. Sharks / IceZilla: Weekly Friday rotation
...
export default function App() {
  ...
  // Real-time Today Date Reference
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-start p-2 sm:p-4 font-sans text-gray-100 select-none">
      ...
      <div className="w-full max-w-6xl bg-gray-900 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden border border-gray-800 flex flex-col">
        ...
        {/* Touch Pinch Zoom & Drag Container */}
        <div 
            ...
            >
              ...
              <div className="grid grid-cols-7 gap-1.5">
                ...
                {days.map(day => {
                  ...
                  const isToday = currDateObj.toDateString() === realToday.toDateString();
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                        {/* Real-time Today Highlight Ring */}
                        <span className={`text-xs sm:text-sm font-bold w-6 h-6 flex items-center justify-center rounded-full
                          ${isToday ? 'bg-pink-500 text-white shadow-md ring-2 ring-pink-300 ring-offset-1 ring-offset-gray-900 font-black' : isSunday ? 'text-red-400' : isSaturday ? 'text-blue-400' : 'text-gray-300'}
                        `}>
                          {day}
                        </span>
                        <button 
        ...
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 z-50 animate-fadeIn">
          <div className="bg-gray-900 border border-gray-700 w-full max-w-md rounded-2xl p-5 shadow-2xl text-gray-100 flex flex-col gap-3.5 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-col gap-2">
              ...
              {/* Custom Time Micro Shift Controls */}
